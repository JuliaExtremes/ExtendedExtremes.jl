...
function bias_correct(obs_ts::Array{Float64,1},  # Série temporelle des observations
    ...
    fm_obs = EGPpowerfit(obs_ts, censoring=2)  # Ajustement du modèle sur obs (la censure sera un hyperparamètre éventuellement)
    fm_sim, fms_sim = EGPpowerfit(sim_ts, sim_covariate, censoring=1);  # Ajustement du modèle ns sur simu (la censure sera un hyperparamètre éventuellement)
